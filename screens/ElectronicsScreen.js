...
export default function ElectronicsScreen() {
  ...
  const [cartItems, setCartItems] = useState([]); // Quản lý các sản phẩm trong giỏ hàng
  ...
  const addToCart = (product) => {
    ...
    if (!existingItem) {
      setCartItems([...cartItems, product]); // Thêm sản phẩm vào giỏ hàng nếu chưa có
    }
  };
  ...
  const goToCart = () => {
    ...
    navigation.navigate('Cart', { cartItems, updateCartItems });
  };
